...
def get_unemployment_rate():
    """
    실업률 (Unemployment Rate) 데이터를 크롤링합니다.
    investing.com에서 실업률 데이터를 가져와 표준 형식으로 반환합니다.
    """
    ...
    try:
        ...
        print(f"실업률 데이터 처리 중 오류 발생: {e}")
        return {"error": f"데이터 처리 실패: {str(e)}"}

def parse_unemployment_rate_data(soup):
    """
    실업률 페이지의 HTML을 파싱하여 최신 데이터를 추출합니다.
    """
    try:
        # 최신 발표 데이터 찾기
        ...
        print(f"실업률 데이터 파싱 중 오류: {e}")
        return {"error": f"데이터 파싱 실패: {str(e)}"}

if __name__ == "__main__":
    print("실업률 데이터 테스트 중...")
    result = get_unemployment_rate()
    print("결과:", result)