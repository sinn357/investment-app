...
const getColorForValue = (actual: string | number | null, forecast: string | number | null): string => {
  ...
  // 기본적으로 예측보다 높으면 초록색(좋음), 낮으면 빨간색(나쁨)
...
export default function PolicyDataSection({ refreshTrigger }: { refreshTrigger: number }) {
  const [activeTab, setActiveTab] = useState('fomc-minutes');
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-lg overflow-hidden">
      ...
      >
        <div className="flex justify-between items-center">
          <div>
            <h3 className="text-xl font-bold">정책지표 데이터</h3>
            <p className="text-indigo-100 mt-1">과거 발표 데이터와 차트 분석</p>
          </div>
          {isExpanded ? (
